...
def get_paging_application_type(rq):
    try:
...
def get_application_host_rel_paging(req):
    ...
    #    start_time=create_time.split("~")[0]
    #    end_time=create_time.split("~")[1]
    sql='  SELECT t1.id,t1.app_id,t1.host_account_id,t1.app_dir,t1.create_date,t1.create_op,t1.upd_date,t1.`status`,t1.mark,t2.app_name,t2.app_exec_file,t3.host_name,t3.host_ip,t3.account_name \
    ...
    if host_name != None and host_name != "":
        sql += ' and t3.host_name like %'+host_name+'%'
    if host_ip != None and host_ip != "":
        sql += ' and t3.host_ip like %'+host_ip+'%'
    if account_name != None and account_name != "":
        sql += ' and t3.account_name like %'+account_name+'%'
    sql += ' limit '+str(startPos)+','+str(endPos)
    """
    searchCondition = {}
    if app_name !=None and app_name != "" :
